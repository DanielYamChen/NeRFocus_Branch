...
flags.DEFINE_bool(
  'eval_once', True,
  'If True, evaluate the model only once, otherwise keeping evaluating new checkpoints if any exist.'
)
flags.DEFINE_bool(
  'save_output', True,
...
def main(unused_argv):
  ...
  dataset = datasets.get_dataset('test', FLAGS.data_dir + config.obj_name, config)
  ...
  optimizer = TrainState.create(
    ...
    tx=optax.adam(learning_rate=config.lr_init),
  )
  ...
  while True:
    ...
    for i in range(n_frames):
      ...
      a = a1 * (1 - theta) + a2 * theta # aperture
      f = f1 * (1 - theta) + f2 * theta # focal length 
      # l = l1**(1 - theta) * l2**theta # focus distance
      l = l1 * (1 - theta) + l2 * theta # focus distance
      ...
      pred_color, pred_distance, pred_acc = models.render_image(
        functools.partial(render_eval_pfn, state.optimizer.params),
        rrays,
        None,
      ...
      if (FLAGS.save_output and (config.test_render_interval > 0)):
        if (idx % config.test_render_interval == 0):
          ...
          utils.save_img_uint8(pred_color, path.join(out_dir, 'color_{:03d}.png'.format(i)))
          # utils.save_img_float32(pred_distance, path.join(out_dir, 'distance_{:03d}.tiff'.format(i)))
          # utils.save_img_float32(pred_acc, path.join(out_dir, 'acc_{:03d}.tiff'.format(i)))
          # for k, v in vis_suite.items():
          #   utils.save_img_uint8(v, path.join(out_dir, k + '_{:03d}.png'.format(i)))
